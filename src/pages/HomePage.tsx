    ...
    <div>
        ...
        <motion.div style={{ y: heroY, scale: heroScale }} className="absolute inset-0">
          ...
          {isDark ? (
            ...
            <>
              ...
              <div className="absolute inset-0 bg-gradient-to-b from-transparent via-transparent to-[#FAFBFC]" />
            </>
          )}
        </motion.div>
        ...
        <motion.div style={{ opacity: heroOpacity }} className="relative max-w-7xl mx-auto px-4 sm:px-6 w-full">
          <div className="max-w-3xl">
            ...
            <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.7 }}
              ...
              <Link to="/catalog" className={`inline-flex items-center justify-center gap-3 px-10 py-5 liquid-glass rounded-2xl text-lg font-bold transition-all ${isDark ? 'hover:bg-white/10' : 'hover:bg-white/90'}`}>
                {t('hero.catalog')}
              </Link>
            </motion.div>
          </div>
      ...
      <section ref={hScrollRef} className="py-28 md:py-36 overflow-hidden">
        ...
        <motion.div style={{ x: hX }} className="flex gap-6 pl-[5vw]">
          {series.map((s) => (
            <Link to="/catalog" key={s.name} className={`group shrink-0 w-[300px] md:w-[360px] rounded-3xl border border-border ${isDark ? 'bg-[#1A1A1F]' : 'bg-white'} hover:border-accent/20 overflow-hidden transition-all duration-500 card-hover`}>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {advantages.map((adv, i) => (
              <motion.div key={adv.title} initial={{ opacity: 0, y: 60, rotateX: 15 }} whileInView={{ opacity: 1, y: 0, rotateX: 0 }} viewport={{ once: true }}
                ...
                <Card3D className={`group p-8 rounded-3xl border border-border hover:border-accent/30 transition-colors cursor-default relative overflow-hidden h-full ${isDark ? 'bg-[#1A1A1F]' : 'bg-white'}`}>
      ...
      <section className="py-20 overflow-hidden">
        ...
        {[false, true].map((rev, ri) => (
          <div key={ri} className="relative mb-4">
            <div className="absolute left-0 top-0 bottom-0 w-24 z-10" style={{ background: `linear-gradient(to right, ${isDark ? '#0C0C0E' : '#FAFBFC'}, transparent)` }} />
            <div className="absolute right-0 top-0 bottom-0 w-24 z-10" style={{ background: `linear-gradient(to left, ${isDark ? '#0C0C0E' : '#FAFBFC'}, transparent)` }} />
            <div className="flex animate-marquee" style={{ animationDirection: rev ? 'reverse' : 'normal', animationDuration: rev ? '40s' : '30s' }}>
              {[...partnerNames, ...partnerNames].map((p, i) => (
                <div key={`${ri}-${i}`} className={`shrink-0 mx-2 px-6 py-3 rounded-2xl border border-border text-sm font-semibold text-text-secondary whitespace-nowrap hover:border-accent/30 hover:text-accent transition-all ${isDark ? 'bg-[#1A1A1F]' : 'bg-white'}`}>
                  {p}
                </div>
              ))}
            </div>
      ...
      {/* ═══════════ INTERIORS ═══════════ */}
      <section className={`py-28 md:py-36 ${isDark ? 'bg-[#141418]' : 'bg-bg-warm'}`}>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {testimonials.map((tt, i) => (
              <motion.div key={tt.name} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                ...
                className={`p-8 rounded-3xl border border-border h-full flex flex-col ${isDark ? 'bg-[#1A1A1F]' : 'bg-white'}`}>
