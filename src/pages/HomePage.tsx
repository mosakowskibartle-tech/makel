    ...
    <div>
        ...
        <motion.div style={{ opacity: heroOpacity }} className="relative max-w-7xl mx-auto px-4 sm:px-6 w-full">
          <div className="max-w-3xl">
            ...
            <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.7 }}
              ...
              <Link to="/catalog" className={`inline-flex items-center justify-center gap-3 px-10 py-5 liquid-glass rounded-2xl text-lg font-bold transition-all hover:bg-white/10`}>
                {t('hero.catalog')}
              </Link>
            </motion.div>
          </div>
            ...
            <div className="grid lg:grid-cols-2 gap-12 w-full items-center">
              ...
              <div className="relative h-[300px]">
                <motion.div style={{ opacity: t1 }} className="absolute inset-0 flex flex-col justify-center">
                  <p className="text-5xl md:text-7xl font-extrabold text-text-primary tracking-tighter">{t('sticky.t1')}</p>
                  <p className="text-xl md:text-2xl font-serif italic mt-4 text-text-secondary">{t('sticky.s1')}</p>
                  <p className="text-sm text-text-muted mt-6 max-w-md">Более 1300 наименований электроустановочных изделий. Каждая серия — произведение инженерного искусства.</p>
                </motion.div>
                <motion.div style={{ opacity: t2 }} className="absolute inset-0 flex flex-col justify-center">
                  <p className="text-5xl md:text-7xl font-extrabold text-text-primary tracking-tighter">{t('sticky.t2')}</p>
                  <p className="text-xl md:text-2xl font-serif italic mt-4 text-text-secondary">{t('sticky.s2')}</p>
                  <p className="text-sm text-text-muted mt-6 max-w-md">Завод площадью 45 000 м², собственная лаборатория VDE, экспорт в 40+ стран мира.</p>
                </motion.div>
                <motion.div style={{ opacity: t3 }} className="absolute inset-0 flex flex-col justify-center">
                  ...
                  <p className="text-xl md:text-2xl font-serif italic mt-4 text-text-secondary">{t('sticky.s3')}</p>
                  <p className="text-sm text-text-muted mt-6 max-w-md">Присоединяйтесь к 47+ партнёрам по всей России. Лучшие условия на рынке.</p>
                </motion.div>
              </div>
              ...
              <div className="flex items-center justify-center">
                <motion.div
                  ...
                  className="w-64 h-80 md:w-80 md:h-[28rem] rounded-none overflow-hidden"
                >
      ...
      <section ref={hScrollRef} className="py-28 md:py-36 overflow-hidden">
        ...
        <motion.div style={{ x: hX }} className="flex gap-6 pl-[5vw]">
          {series.map((s) => (
            <Link to="/catalog" key={s.name} className={`group shrink-0 w-[300px] md:w-[360px] rounded-3xl border border-border bg-bg-card hover:border-accent/20 overflow-hidden transition-all duration-500 card-hover`}>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {advantages.map((adv, i) => (
              <motion.div key={adv.title} initial={{ opacity: 0, y: 60, rotateX: 15 }} whileInView={{ opacity: 1, y: 0, rotateX: 0 }} viewport={{ once: true }}
                ...
                <Card3D className={`group p-8 rounded-3xl border border-border hover:border-accent/30 transition-colors cursor-default relative overflow-hidden h-full bg-bg-card`}>
      ...
      <section className="bg-bg-warm relative overflow-hidden">
        <div className="absolute inset-0 opacity-5">
          <img src={images.about.techFactory} alt="" className="w-full h-full object-cover" />
        </div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 grid lg:grid-cols-2 gap-16 items-center">
            <AnimatedSection>
              <h2 className="text-4xl md:text-5xl font-extrabold tracking-tight leading-tight text-text-primary">
                ...
                <span className="font-serif italic font-normal text-text-muted">мощнее любого конкурента</span>
              </h2>
              ...
              <div className="mt-8 grid grid-cols-3 gap-6">
                {[
                  ...
                  <div key={it.l}><it.i className="text-accent mb-2" size={24} /><p className="text-sm font-bold text-text-primary">{it.l}</p><p className="text-xs text-text-muted">{it.v}</p></div>
                ))}
              </div>
            </AnimatedSection>
            <AnimatedSection delay={0.3} direction="left">
              <div className="relative">
                ...
                <img src={images.about.factory} alt="" className="relative rounded-[2rem] w-full aspect-[4/3] object-cover border border-border" />
      ...
      <section className="py-20 overflow-hidden">
        ...
        {[false, true].map((rev, ri) => (
          <div key={ri} className="relative mb-4">
            <div className="absolute left-0 top-0 bottom-0 w-24 z-10" style={{ background: 'linear-gradient(to right, #FAFBFC, transparent)' }} />
            <div className="absolute right-0 top-0 bottom-0 w-24 z-10" style={{ background: 'linear-gradient(to left, #FAFBFC, transparent)' }} />
            <div className="flex animate-marquee" style={{ animationDirection: rev ? 'reverse' : 'normal', animationDuration: rev ? '40s' : '30s' }}>
              {[...partnerNames, ...partnerNames].map((p, i) => (
                <div key={`${ri}-${i}`} className={`shrink-0 mx-2 px-6 py-3 rounded-2xl border border-border text-sm font-semibold text-text-secondary whitespace-nowrap hover:border-accent/30 hover:text-accent transition-all bg-bg-card`}>
                  {p}
                </div>
              ))}
            </div>
      ...
      {/* ═══════════ INTERIORS ═══════════ */}
      <section className={`py-28 md:py-36 bg-bg-warm`}>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[
              ...
              <motion.div key={item.label} initial={{ opacity: 0, y: 60, scale: 0.95 }} whileInView={{ opacity: 1, y: 0, scale: 1 }} viewport={{ once: true }}
                ...
                <div className="absolute bottom-6 left-6"><span className="liquid-glass-strong px-5 py-2.5 rounded-xl text-sm font-bold text-text-primary">{item.label}</span></div>
              </motion.div>
            ))}
          </div>
        </div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {testimonials.map((tt, i) => (
              <motion.div key={tt.name} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                ...
                className={`p-8 rounded-3xl border border-border h-full flex flex-col bg-bg-card`}>
      ...
      <section className="relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-accent/5 via-bg-warm to-accent/10" />
        <div className="absolute inset-0 dot-pattern opacity-20" />
        <div className="relative py-28 md:py-36 text-center">
          <div className="max-w-4xl mx-auto px-4 sm:px-6">
            <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}>
              <p className="text-lg font-bold uppercase tracking-[0.2em] text-accent/60 mb-6">{t('cta.ready')}</p>
              <h2 className="text-4xl md:text-6xl lg:text-7xl font-extrabold tracking-tight leading-tight text-text-primary">
                {t('cta.title1')}<br /><span className="font-serif italic font-normal text-text-muted">{t('cta.title2')}</span>
              </h2>
              <p className="mt-8 text-xl text-text-secondary max-w-2xl mx-auto">{t('cta.desc')}</p>
              <div className="mt-12 flex flex-col sm:flex-row gap-4 justify-center">
                <Link to="/contacts" className="group inline-flex items-center justify-center gap-3 px-12 py-6 bg-accent hover:bg-accent-light text-white rounded-2xl text-xl font-extrabold transition-all hover:shadow-2xl hover:shadow-accent/20">
                ...
                <a href="https://t.me/makel_russia" target="_blank" rel="noopener noreferrer"
                  className="inline-flex items-center justify-center gap-3 px-12 py-6 bg-white hover:bg-surface border border-border text-text-primary rounded-2xl text-xl font-bold transition-all">
