      ...
      {cities.map((city) => {
        ...
        return (
          <motion.div key={city.city} className="absolute cursor-pointer" style={{ left: `${pos.x}%`, top: `${pos.y}%`, transform: 'translate(-50%,-50%)' }}
            ...
            {city.count > 1 && <span className="absolute -top-1 -right-2 bg-white text-accent text-[7px] font-extrabold rounded-full w-4 h-4 flex items-center justify-center z-20 shadow border border-accent/20">{city.count}</span>}
          </motion.div>
        );
      })}
